...
export const CreationForm = () => {
  ...
  //       return acc;
  //     },
  //     { fields: [], associatedText: formData.associatedText } // Initialize acc to match InputDataType
  //   );
  //   return newInputData;
  // };
// Add domain, subtest and child subtest as well as update the form data
  ...
  const handleAddChildSubtest = (index: number) => {
    ...
    if (parentDomain.type === "domain") {
      // Directly update the domain's subtests array
  ...
  const handleRemove = (index: number) => {
    ...
    if (currentField.type === "domain") {
      ...
      fields.forEach((field, idx) => {
        if (field.type === "subtest" && field.fieldData.id === currentField.id) {
      ...
      const parentDomainIndex = fields.findIndex(field => 
        field.type === "domain" && 
        field.subtests?.some(subtest => subtest.id === currentField.fieldData.id)
      );
                    ...
                    <div key={subtest.id} className="flex gap-4 items-center">
                      ...
                      <div className="flex-1 flex gap-4">
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem className="flex-1">
                              <FormLabel>Subtest Name {subIndex + 1}</FormLabel>
                              <FormControl>
                                <Input
